...
namespace O::Configuration::Application
{
	 ...
	 *
	 * Template parameters:
	 *  - DataModules... : the concrete data types for each module.
	 *
	 * The container exposes the raw tuple as the member `modules` so callers
	 * may iterate, access or move the contained modules. Use `Get<T>()` to
	 * obtain a reference to the module by its type.
	 */
	...
	struct Container
	{
		 ...
		 *
		 * @note This uses std::get<T> and therefore will fail to compile if
		 *       T is not part of DataModules...
		 */
		template<class T>
		T& Get()
		{
